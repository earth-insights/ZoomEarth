...
# - The bounding box [x_min, y_min, x_max, y_max] of a cropped region  
# - A natural language question referring to the image  
# - The ground truth answer  

# Important:  
# - The cropped region (given by the bounding box) is the **key area that contains the information required to answer the question**.  
# - The global image should only be used for context and to explain how one would locate the cropped region.  
# - The final answer must be derived **only by analyzing the cropped region**.  
# - When writing <stage_1_reasoning>, you must explicitly reference the given bounding box as the final localization result, and you must output the bounding box in the following JSON format:  

# ```json
# [
#     {{"bbox_2d": [x_min, y_min, x_max, y_max], "label": "<short description of the localized region>"}}
# ]
# ```
...
# <global>  
# - Provide a brief but informative description of the global satellite image (e.g., main structures, spatial layout).  
# <\global>  
...
# - Localization Strategy: Parse the question to extract key terms (such as target category, color, relative position, size/scale cues). Translate these into a visual query; scan the global image using attention mechanisms, sliding windows, object detection, semantic segmentation, or feature-matching retrieval methods to generate candidate regions and compute their relevance scores with respect to the question. Apply spatial and semantic constraints to filter and rank the candidates, producing the final localization result.  
# - Reasoning Result: Through this process, the model identifies and outputs the final cropped region, represented by the bounding box **[x_min, y_min, x_max, y_max]**. This bounding box is selected because the area spatially aligns with the positional or scale-related hints in the question and visually exhibits features (such as expected shapes, structures, colors/spectral signatures, textures, or arrangements) consistent with the query terms. Therefore, this region is determined to be the key area containing the necessary information to answer the question. **Note: The final answer is not revealed at this stage; only the localization reasoning is described.**
# <\\stage_1_reasoning>  

# <stage_2_reasoning>  
# - Given the cropped region (the key area for answering), explain how to analyze it to derive the final answer.  
# - Specify what visual features should be observed (e.g., shape, number, color, spatial arrangement).  
# <\\stage_2_reasoning>  
...
# - The <stage_2_reasoning> must directly connect the cropped region to the answer.

# Input:
# - Question: {result["question"]}
# - Ground Truth Answer: {result["ground_truth"]}
# - Bounding box: {[int(x / scale) for x in hbox]}
# """
...
#   * In **Localization Strategy**, only describe the approximate location of the relevant region in natural language (e.g., "the rectangular green farmland in the lower right"). Do not output exact coordinates here.  
#   * In **Reasoning Result**, you must output the bounding box in the following JSON format, and nothing else:  
...
# Localization Strategy: Parse the question to extract key terms (such as target category, color, relative position, size/scale cues). Translate these into a visual query; scan the global image using attention mechanisms, sliding windows, object detection, semantic segmentation, or feature-matching retrieval methods to generate candidate regions and compute their relevance scores with respect to the question. Apply spatial and semantic constraints to filter and rank the candidates. Summarize the approximate location of the final selected region (e.g., "a rectangular cluster of bright rooftops in the upper left").

# Reasoning Result: Output only the JSON-formatted bounding box and a short label, as specified above. Do not include any additional explanation here.
# <\stage_1_reasoning>
...
def main(image_dir, jsonl_in, jsonl_out, model_name="qwen2-vl-72b-instruct", max_workers=2):
...
if __name__ == "__main__":
    main("LRS_VQA/image", "LRS_VQA/LRS_VQA_merged.jsonl", "dataset/vl_answers_v2.jsonl")
