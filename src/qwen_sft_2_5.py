...
from model import Qwen2_5_VLProcessor
...
os.environ["CUDA_VISIBLE_DEVICES"] = "4"  # 指定单个 GPU

@dataclass
class TrainingConfig:
    model_name: str = "/home/zhangjunjie/.cache/huggingface/hub/models--Qwen--Qwen2.5-VL-3B-Instruct/snapshots/66285546d2b821cf421d4f5eb2576359d3770cd3"
    dataset_name_train: str = "./LRS_VQA_RL_V2/sft"
    dataset_name_val: str = "./LRS_VQA_RL_V2/test"
    output_dir: str = "output_v2_sft1000"
    img_folder = "/home/zhangjunjie/CaoXiongyong_Student/fubowen/LRS_VQA/LRS_VQA/image"
...
<|im_start|>user
<|vision_start|><|image_pad|><|vision_end|>""" + example["question"].split("Answer the question using a single word or phrase.")[0] + """
...
"""  # <image> how are you ?  # <|placeholder|> <|placeholder|> ...<|placeholder|>  how are you ?
        ...
        if example['cut']:
            label = (
                f"<think>{example['global']} "
                f"{example['stage_1_reasoning']}</think>\n"
                f"<|image_pad|>\n"
                f"<think>{example['stage_2_reasoning']}</think>\n"
            ...
            label = (
                ...
                f"{example['stage_2_reasoning']}</think>\n"
                f"<answer>{example['ground_truth']}</answer> <|endoftext|>"
            )
            # 只用一张全图图像
    ...
    tokens = processor(text=texts, images=images, return_tensors="pt", padding="longest", text_pair=labels, max_length=4096)
    # print('hello')
    return tokens.to(dtype), None  # 你可以用 id 也可以不返回


def prepare_model_and_processor(config: TrainingConfig):
    processor = Qwen2_5_VLProcessor.from_pretrained(config.model_name, max_pixels = config.max_pixels)
