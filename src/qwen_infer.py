...
def record(fout, prompt, sample, data, output1, output2, is_error):
    fout.write(json.dumps({
        ...
        "location_ref":   data["area"],  
        "location":       extract_location(output1),
        "prompt":         prompt,
        ...
        "image":          sample["image"],
        "error":          is_error,
        "model_id":       "Qwen2-VL-7B-Instruct+LoRA"
    }, ensure_ascii=False) + "\n")
...
def eval_model_lora():
    ...
    out_path = "results/lora_infer.jsonl"
...
"""
    for ex in tqdm(test_ds):
        try:
            ...
            cur_prompt = sample["text"].split("Answer the question using a single word or phrase.")[0]
            ...
            output1 = chat(prompt, [img], processor, accelerator, model, stop_criteria)

            # stage 2: Question + Image (downsampled) + previous reasoning + Image (cropped)
            prompt =  prompt + output1.split("<answer>")[0] + "<|image_pad|>"
            bbox = extract_bbox(output1, scale)
            if not bbox:
                record(fout, cur_prompt, sample, ex, output1, "", True)
                continue
            image_bbox = Image.open(image_fp).convert("RGB")
            image_bbox = cut_image(image_bbox, bbox)
            image_bbox = resize_image(image_bbox)

            output2 = chat(prompt, [img, image_bbox], processor, accelerator, model, stop_criteria)

            record(fout, prompt, sample, ex, output1, output2, False)
        except:
            record(fout, prompt, sample, ex, "", "", True)
    fout.close()
