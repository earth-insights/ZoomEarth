...
def End(vimd="liurx2-0"):  
    name = "liurx"
    password = "Lrx123345567"
    vmids = [vimd]
...
def score_one(completion1:str, completion2:str, ground_truth:dict) -> float:
    ...
    if isinstance(location_ref, str) and isinstance(location, str):
        if location_ref == "center-":
            loc_reward += 1.0 if location == "center-" else 0.0
        else:
    ...
    return pattern_reward, bbox_reward, answer_reward, loc_reward, float(pattern_reward + bbox_reward + answer_reward + loc_reward)
...
def save_checkpoint(accelerator, model, epoch, step, loss, checkpoint_dir="./output_rl"):
    checkpoint_dir = f"{checkpoint_dir}/checkpoint-{step}"
...
def main():
    prev_time = time.time()
    accelerator = Accelerator(mixed_precision="bf16", project_dir="qwen_rl_log", gradient_accumulation_steps=NUM_GENERATIONS, log_with=["tensorboard"])

    accelerator.init_trackers(
        f"Qwen-RL-{date.today()}{datetime.now().time()}".replace(":", "_"),
        init_kwargs={
    ...
    MAX_STEPS = MAX_ITER * len(dl) * GRPO_STEPS
        ...
        for example in dl: 
            try:
                ...
                for i in range(len(completions1)):
                    pattern_reward, bbox_reward, answer_reward, loc_reward, reward = score_one(completions1[i], completions2[i], example)
                    pattern_rewards.append(pattern_reward)
                    bbox_rewards.append(bbox_reward)
                ...
                loc_rewards = torch.tensor(loc_rewards, device=accelerator.device, dtype=torch.float32)
                answer_rewards = torch.tensor(answer_rewards, device=accelerator.device, dtype=torch.float32)
                
                pattern_reward_mean = pattern_rewards.mean()
                bbox_reward_mean = bbox_rewards.mean()
                ...
                with torch.no_grad():
                    ...
                    torch.cuda.empty_cache()
                    # print("\n\nstage2 done!\n\n")
                # print(f"计算old/ref概率用时: {time.time() - prev_time} s")
                prev_time = time.time()
                ...
                for _ in range(GRPO_STEPS):
                    for i in range(NUM_GENERATIONS):
                        with accelerator.accumulate(theta):  
                            ...
                            # ratio_kl = approx_kl.exp()
                            # kl_loss = ((ratio_kl - 1) - approx_kl).mean()
                        ...
                        if accelerator.sync_gradients:
                            ...
                            if global_step % LOG_EVERY == 0:
                                ...
                                reward_mean_world = (accelerator.gather(reward_mean).sum().item() /
                                                max(1, accelerator.num_processes) / max(1, running_count))

                                pattern_reward_mean_world = (accelerator.gather(pattern_reward_mean).sum().item() /
                                                max(1, accelerator.num_processes) / max(1, running_count))

                                bbox_reward_mean_world = (accelerator.gather(bbox_reward_mean).sum().item() /
                                                max(1, accelerator.num_processes) / max(1, running_count))

                                answer_reward_mean_world = (accelerator.gather(answer_reward_mean).sum().item() /
                                                max(1, accelerator.num_processes) / max(1, running_count))

                                loc_reward_mean_world = (accelerator.gather(loc_reward_mean).sum().item() /
                                                max(1, accelerator.num_processes) / max(1, running_count))
                                ...
                                accelerator.log(
                                    {
                                        ...
                                        "train/kl": kl_penalty.detach().float().item(),
                                        "train/learning_rate": sched.get_last_lr()[0],
                                        "train/grad_norm": gnorm,
                                        "reward/mean": reward_mean_world,
                                        "reward/pattern": pattern_reward_mean_world,
                                        "reward/bbox": bbox_reward_mean_world,
                                        "reward/location": loc_reward_mean_world,
                                        "reward/answer": answer_reward_mean_world,
