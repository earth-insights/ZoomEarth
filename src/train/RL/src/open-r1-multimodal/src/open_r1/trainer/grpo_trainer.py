...
from open_r1.custom import customized_funcs
...
class VLMGRPOTrainer(Trainer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        trainable_params_cnt = sum(p.numel() for p in trainable_params)
        total_params_cnt = sum(p.numel() for p in model.parameters())
        # for n, p in model.named_parameters():
        #     if p.requires_grad:
        #         print(n, p.shape)
        print(f"Total trainable parameters: {trainable_params_cnt}")
        print(f"Total parameters: {total_params_cnt}")
    ...
    def _generate_and_score_completions(self, inputs: dict[str, Union[torch.Tensor, Any]], model) -> dict[str, Union[torch.Tensor, Any]]:
        ...
        image_paths = [x["image_path"] for x in inputs]
        ...
        images = []
        for x in inputs:
            imgs = [customized_funcs.resize_image(PIL.Image.open(p).convert("RGB")) for p in self._get_key_from_inputs(x, "image_path")]
            images.append(imgs)
        ...
        with unwrap_model_for_generation(model, self.accelerator) as unwrapped_model:
            ...
            completions_2 = []
            attention_mask2 = []
            prompt_completion_ids_2 = []
            stage_2_prompts = []
            stage_2_imgs = []
            # stage 2
            for idx, (input, completion, imgs, stage_1_prompt, attention_mask_item, generate_returned_result_item) in enumerate(zip(inputs, completions, images, prompts, attention_mask, generate_returned_result)):
                if input['bbox'] != []:
                    bbox = customized_funcs.extract_bboxes(completion)
                    if bbox == []:
                        bbox = [0,0,imgs[0].width, imgs[0].height]
                    else:
                        bbox = bbox[0]
                    image = PIL.Image.open(image_paths[idx][0]).convert("RGB")
                    scale = max(max(image.width, image.height) / 512, 1)
                    input['scale'] = scale
                    bbox = [point * scale for point in bbox]
                    image_bbox = customized_funcs.resize_image(customized_funcs.cut_image(image, bbox))
                    stage_2_images = [imgs[0], image_bbox]
                    stage_2_prompt = stage_1_prompt + completion.split("<answer>")[0] + "<|vision_start|><|image_pad|><|vision_end|>"
                    stage_2_prompts.append(stage_2_prompt)
                    stage_2_imgs.append(stage_2_images)
                    generate_returned_result2, prompt_inputs2 = customized_funcs.chat(stage_2_prompt, stage_2_images, self.processing_class, self.accelerator, model)
                    prompt_mask2 = prompt_inputs2['attention_mask']
                    prompt_ids2 = prompt_inputs2['input_ids']
                    prompt_length2 = prompt_ids2.size(1)

                    if not self.vlm_module.is_embeds_input():
                        prompt_completion_ids2 = generate_returned_result2
                        prompt_ids2 = prompt_completion_ids2[:, :prompt_length2]
                        completion_ids2 = prompt_completion_ids2[:, prompt_length2:]
                    else:
                        # In this case, the input of the LLM backbone is the embedding of the combination of the image and text prompt
                        # So the returned result of the `generate` method only contains the completion ids
                        completion_ids2 = generate_returned_result2
                        prompt_completion_ids = torch.cat([prompt_ids2, completion_ids2], dim=1)

                    is_eos = prompt_completion_ids2 == 151643
                    eos_idx = torch.full((is_eos.size(0),), is_eos.size(1), dtype=torch.long, device=device)
                    eos_idx[is_eos.any(dim=1)] = is_eos.int().argmax(dim=1)[is_eos.any(dim=1)]
                    sequence_indices = torch.arange(is_eos.size(1), device=device).expand(is_eos.size(0), -1)
                    completion_mask2 = (sequence_indices <= eos_idx.unsqueeze(1)).int()

                    # Concatenate prompt_mask with completion_mask for logit computation
                    attention_mask = completion_mask2
                    attention_mask2.append(attention_mask[0])
                    prompt_completion_ids_2.append(prompt_completion_ids2[0])
                    completions_2.append(self.processing_class.tokenizer.decode(completion_ids2[0],skip_special_tokens=False).strip())
                else:
                    input['scale'] = 1
                    prompt_completion_ids_2.append(generate_returned_result_item)
                    attention_mask2.append(torch.as_tensor(attention_mask_item, dtype=torch.long, device=completion_ids.device))
                    completions_2.append("")
                    stage_2_prompts.append(prompts[idx])
                    stage_2_imgs.append(images[idx])

        attention_mask2_padded = pad_sequence(attention_mask2, batch_first=True, padding_value=0)
        final_prompt_completion_ids = pad_sequence(prompt_completion_ids_2, batch_first=True, padding_value=self.processing_class.pad_token_id)
        final_completion_ids = final_prompt_completion_ids[:, prompt_length:]
        final_attention_mask = attention_mask2_padded
        final_completion_mask = final_attention_mask[:, prompt_length:]
        
        final_inputs = self.processing_class(
            text=stage_2_prompts,
            images=stage_2_imgs,
            return_tensors="pt",
            padding="longest",
        ...
        final_multimodal_inputs = {k: final_inputs[k] if k in final_inputs else None for k in multimodal_keywords}
        ...
        with torch.no_grad():
            ...
            if self.num_iterations > 1:
                old_per_token_logps = self._get_per_token_logps(
                    model, final_prompt_completion_ids, final_attention_mask, **final_multimodal_inputs
                )
            ...
            if self.beta == 0.0:
                ...
                ref_per_token_logps = self._get_per_token_logps(
                    self.ref_model, final_prompt_completion_ids, final_attention_mask, **final_multimodal_inputs
                )
            else:
        ...
        for i, (reward_func, reward_processing_class) in enumerate(
            ...
            if isinstance(reward_func, PreTrainedModel):
                ...
                output_reward_func = reward_func(prompts=prompts, completion1=completions, completion2=completions_2, **reward_kwargs)
        ...
        # Log the metrics
        completion_length = self.accelerator.gather_for_metrics(final_completion_mask.sum(1)).float().mean().item()
        ...
        return {
            ...
            "multimodal_inputs": final_multimodal_inputs
        }
