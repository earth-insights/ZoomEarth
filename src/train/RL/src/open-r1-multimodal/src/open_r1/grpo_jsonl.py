...
import torch
from custom import customized_funcs
...
reward_funcs_registry = {
    "answer": customized_funcs.get_answer_reward,
    "iou": customized_funcs.get_bbox_reward,
    "format": customized_funcs.get_format_reward
}
    ...
    for data_file, image_folder, accu_reward_method in zip(data_files, image_folders, accu_reward_methods):
        with open(data_file, 'r') as f:
            for line in f:
                ...
                item['question'] = question_prompt.format(Question=item['question'])
                
                item['accu_reward_method'] = item.get('accu_reward_method', accu_reward_method)
                all_data.append(item)
    dataset = Dataset.from_list(all_data)
    dataset = dataset.select(range(2400))
