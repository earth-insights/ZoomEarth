    ...
    def __call__(
        ...
        **kwargs: Unpack[Qwen2VLProcessorKwargs],
        ...
        if images is not None:
            image_inputs = self.image_processor(images=images, **output_kwargs["images_kwargs"])   #image [1024,1024] output: 
        ...
        if images is not None:
            ...
            for i in range(len(text)):
                while self.image_token in text[i]:  #<image>
            ...
            for i in range(len(output_kwargs["text_kwargs"]['text_pair'])):   # 把A中的图片标记替换成标志位
                while self.image_token in output_kwargs["text_kwargs"]['text_pair'][i]:  #<image>
                    ...
                    index += 1
                output_kwargs["text_kwargs"]['text_pair'][i] = output_kwargs["text_kwargs"]['text_pair'][i].replace("<|placeholder|>", self.image_token)
