...
class Qwen2VisionTransformerPretrainedModel(Qwen2VLPreTrainedModel):
    ...
    def rot_pos_emb(self, grid_thw):
        pos_ids = []
        for t_tensor, h_tensor, w_tensor in grid_thw:

            # t, h, w = t_tensor.item(), h_tensor.item(), w_tensor.item()
            t = int(t_tensor.item())
            h = int(h_tensor.item())
            w = int(w_tensor.item())
    ...
    def forward(
        ...
        **kwargs,
        ...
        cu_seqlens = torch.repeat_interleave(grid_thw[:, 1] * grid_thw[:, 2], grid_thw[:, 0].long()).cumsum(
            dim=0,
...
class Qwen2VLModel(Qwen2VLPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if inputs_embeds is None:
            inputs_embeds = self.get_input_embeddings()(input_ids)  # bs, seq_len, embed_dim
            if pixel_values is not None:
                ...
                inputs_embeds = inputs_embeds.masked_scatter(image_mask, image_embeds)  # 覆盖 <|placeholder|>的embedding
    ...
    def forward(
        ...
        **kwargs: Unpack[KwargsForCausalLM],
        ...
        if labels is not None:
            ...
            loss = loss_fct(flat_logits, flat_labels)   # flat_labels -100 代表图片和Q
